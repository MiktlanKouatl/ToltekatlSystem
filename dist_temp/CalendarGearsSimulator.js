...
function runCalendarGearsSimulation() {
    ...
    for (let year = 1; year <= totalYearsToSimulate; year++) {
        ...
        const yearBearerName = ToltekatlSystem_1.YEAR_BEARERS[(year - 1) % 4];
        const yearBearerNumeral = ((year - 1) % 13) + 1;
        const yearStartTimeHours = ToltekatlSystem_1.YEAR_BEARER_START_TIMES[yearBearerName];
        for (let day = 1; day <= daysInNormalYear; day++) { // Simulate 365 days for each year
            const isNemontemi = day > 360;
            const veintenaName = isNemontemi ? null : ToltekatlSystem_1.VEINTENA_NAMES[Math.floor((day - 1) / 20)];
